...
let hardcoded_settings = {
            "auth": "auth",
            "region": "",
            "bucket": "bucket-1",
            "entered_bucket": "bucket-1",
            "selected_bucket": "",
            "view": "folder",
            "delimiter": "/",
            "prefix": "",
            ...
            "stscred": null,
            "endpoint": "https://s3-demo.wasmer.app",
            "buckets": [
                "bucket-1", "bucket-2"
            ]
        };
...
app.factory('SharedService', function($rootScope) {
    ...
    shared.changeSettings = function(settings) {
        ...
        var me = this;
        this.settings = {...hardcoded_settings, ...settings}
...
app.controller('ViewController', function($scope, SharedService) {
    ...
    $scope.renderObject = function(data, type, full) {
        DEBUG.log("renderObject:", full, data);
    ...
    $scope.renderFolder = function(data, type, full) {
        console.log("renderFolder:", full, data);
...
app.controller('SettingsController', function($scope, SharedService) {
    ...
    $scope.settings = hardcoded_settings;
